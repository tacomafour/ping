...
import com.trailheadsoftware.ping.chunk.types.IHDR.ColorType;
import com.trailheadsoftware.ping.chunk.types.PLTE.PaletteEntry;
import com.trailheadsoftware.ping.chunk.types.iTXt.iTXtChunk;
import com.trailheadsoftware.ping.chunk.types.sPLT.SuggestedPalette;
...
import com.trailheadsoftware.ping.util.ByteUtil;
    ...
    private void write_iTXt() throws IOException
    {
        if (graphic.getItxtChunks().size() > 0)
        {
            for (iTXtChunk chunk : graphic.getItxtChunks())
            {
                ...
                output.write(chunk.getLanguageTag().getBytes(StandardCharsets.US_ASCII));
                output.writeByte((byte) 0);
                output.write(chunk.getTranslationKeyword().getBytes(StandardCharsets.UTF_8));
                output.writeByte((byte) 0);
                output.write(chunk.getTranslatedValue().getBytes(StandardCharsets.UTF_8));
